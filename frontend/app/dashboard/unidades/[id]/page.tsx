...
export default function UnidadeDetalhesPage() {
	...
	const [ultimosFuncionarios, setUltimosFuncionarios] = useState<
		UltimosFuncionariosCadastrados[]
	>([]);
	...
	const atualizarDados = useCallback(async () => {
		...
		try {
			...
			const responsePresencaMensal = await api.get(
				`/unid/unid/${id}/presencas-mensais`,
			);
			...
			const responsePresencasHoje = await api.get(
				`/unid/unid/${id}/registros-hoje`,
			);
			const presencaHoje = Number(
				responsePresencasHoje.data?.total_registros_hoje ?? 0,
			);

			const responseFerias = await api.get("/ferias/ferias-solicitadas/total");
			const feriasData: FeriasSolicitadasPorUnidade[] =
				responseFerias.data.dados;
			const feriasDaUnidade = feriasData.find(
				(f) => f.unidade_id === Number(id),
			);
			const totalFerias = feriasDaUnidade
				? Number(feriasDaUnidade.total_ferias_solicitadas)
				: 0;
	...
	if (!unidade) {
		return (
			<div className="p-4 text-center">
				Unidade não encontrada ou ocorreu um erro.
			</div>
		);
	}
