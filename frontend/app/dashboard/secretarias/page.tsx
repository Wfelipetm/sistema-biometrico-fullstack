  ...
  useEffect(() => {
    const fetchSecretarias = async () => {
      try {
        ...
        const response = await fetch("http://biometrico.itaguai.rj.gov.br:3001/secre")
        if (!response.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie as secretarias cadastradas no sistema</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Total de {secretarias.length} secretarias cadastradas</CardDescription>
        </CardHeader>
        <CardContent>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => router.push(`/dashboard/secretarias/editar/${secretaria.id}`)}
                            >
