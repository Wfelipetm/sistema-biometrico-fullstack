...
export function useRelatorioPDF() {
  ...
  const gerarRelatorioPDF = async (funcionarioId: string, mes: string, ano: string) => {
    ...
    try {
      ...
      // Substitua a parte de geração de registrosCompletos
      ...
      if (logoBase64) {
        doc.addImage(logoBase64, "PNG", margin, 5, 10, 12) // x, y, largura, altura
      }

      doc.setTextColor(...whiteColor)
      doc.setFontSize(12)
      doc.setFont("helvetica", "bold")
      doc.text("RELATÓRIO DE PONTO", margin + 16, 11)

      doc.setFontSize(8)
      doc.setFont("helvetica", "normal")
      doc.text("Sistema de Controle de Ponto Biométrico", margin + 16, 14)

      doc.setFontSize(7)
      const today = formatDateFns(new Date(), "dd/MM/yyyy, HH:mm")
      doc.text(`Emitido em: ${today}`, pageWidth - margin - 40, 9)

      doc.setFontSize(9)
      doc.setFont("helvetica", "bold")
      doc.text("PREFEITURA DE ITAGUAÍ", pageWidth - margin - 40, 14)
      ...
      doc.setFontSize(7)
      doc.setFont("helvetica", "normal")
      doc.text(`Nome: ${data.funcionario.nome.toUpperCase()}`, margin + 2, 28)
      doc.text(`Matrícula: ${data.funcionario.matricula}`, margin + 80, 23)
      doc.text(`Cargo: ${data.funcionario.cargo}`, margin + 120, 23)
      doc.text(`Unidade: ${data.funcionario.unidade_nome}`, margin + 180, 28)
      doc.text(`Escala: ${data.funcionario.tipo_escala}`, margin + 80, 28)
      doc.text(`Referência: ${data.funcionario.mes_ano}`, margin + 120, 28)
